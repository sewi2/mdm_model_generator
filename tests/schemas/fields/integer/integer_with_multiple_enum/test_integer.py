...
class TestModelGeneratorIntegerMultipleEnumField(ModelGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "(models.BigIntegerField if getattr(settings, "
            "'MDM_MODELS_BIGINT_ON', False) "
            "else models.IntegerField)"
            "(verbose_name=_('Перечисление(Целое число)'), "
            'help_text=_("Пример значения \'enum+integer\'"), '
            'blank=True, null=True, '
            "default=None, editable=False, "
            "db_column='test_field' if getattr(settings, "
            "'MDM_MODELS_CAMEL_CASE', False) else None, "
            "choices=TEST_FIELD_CHOICES)")


class TestSerializerGeneratorIntegerMultipleEnumField(
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "serializers.ChoiceField(label=_('Перечисление(Целое число)'), "
            'help_text=_("Пример значения \'enum+integer\'"), '
