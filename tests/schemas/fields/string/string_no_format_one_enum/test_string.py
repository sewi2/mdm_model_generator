...
class TestModelGeneratorCharNotFormattedOneEnumField(ModelGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            ...
            "default=None, editable=False, db_column='test_field' if getattr("
            "settings, 'MDM_MODELS_CAMEL_CASE', False) "
            'else None, choices=TEST_FIELD_CHOICES, max_length=255)')


class TestSerializerGeneratorCharNotFormattedOneEnumField(
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "serializers.ChoiceField(label=_('Type'), "
            "help_text=_('Человекочитаемое "
            'наименование сущности. Совпадает с наименованием Контракта '
            "({entity}.json)'), allow_blank=True, allow_null=True, "
            'choices=base_models.BaseTestModelSerializer.TEST_FIELD_CHOICES)')
