...
class TestModelGeneratorDateField(ModelGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            'models.DateField(verbose_name=_(\'Дата\'), '
            'help_text=_("Пример значения \'string\', формат \'date\'"), '
            'blank=True, null=True, default=None, '
            "editable=False, db_column='test_field' if getattr(settings, "
            "'MDM_MODELS_CAMEL_CASE', False) else None)")


class TestSerializerGeneratorDateField(SerializerGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            'serializers.DateField(label=_(\'Дата\'), '
            'help_text=_("Пример значения '
            '\'string\', формат \'date\'"), allow_null=True)')
