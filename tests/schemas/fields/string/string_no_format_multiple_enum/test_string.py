...
class TestModelGeneratorCharNotFormattedMultipleEnumField(ModelGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "models.CharField(verbose_name=_('Перечисление(Строка)'), "
            'help_text=_("Пример значения \'enum+string\'"), '
            'blank=True, null=True, '
            "default=None, editable=False, db_column='test_field' "
            "if getattr(settings, 'MDM_MODELS_CAMEL_CASE', False) "
            "else None, choices=TEST_FIELD_CHOICES, "
            'max_length=50)')


class TestSerializerGeneratorCharNotFormattedMultipleEnumField(
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "serializers.ChoiceField(label=_('Перечисление(Строка)'), "
            'help_text=_("Пример значения \'enum+string\'"), '
