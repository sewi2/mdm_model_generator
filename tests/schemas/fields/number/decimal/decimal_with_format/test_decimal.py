...
class TestModelGeneratorDecimalFormattedField(ModelGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "models.DecimalField(verbose_name=_('Десятичное число'), "
            'help_text=_("Пример значения \'number\', '
            'формат \'decimal\'"), blank=True, '
            "null=True, default=None, editable=False, db_column='test_field' "
            "if getattr(settings, 'MDM_MODELS_CAMEL_CASE', False) else None, "
            "validators=[MinValueValidator(Decimal('-1000000000000')), "
            "MaxValueValidator(Decimal('1000000000000'))], max_digits=17, "
            'decimal_places=4)')


class TestSerializerGeneratorDecimalFormattedField(SerializerGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            'serializers.DecimalField(label=_(\'Десятичное число\'), '
            'help_text=_("Пример значения \'number\', формат \'decimal\'"), '
            'allow_null=True, max_digits=17, decimal_places=4, '
