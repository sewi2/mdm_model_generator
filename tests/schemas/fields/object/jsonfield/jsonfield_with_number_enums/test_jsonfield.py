...
class TestModelGeneratorJsonNumberEnumsField(ModelGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "models.JSONField(verbose_name=_('Координаты'), "
            "help_text=_('Координаты в формате долгота/широта полученные из "
            "сервиса DaData'), blank=True, null=True, default=dict, "
            "editable=False, db_column='test_field' if "
            "getattr(settings, 'MDM_MODELS_CAMEL_CASE', False) else None)")


class TestSerializerGeneratorJsonNumberEnumsField(SerializerGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "serializers.JSONField(label=_('Координаты'), "
            "help_text=_('Координаты в формате долгота/широта полученные "
            "из сервиса DaData'), allow_null=True, "
            'default=dict)')
