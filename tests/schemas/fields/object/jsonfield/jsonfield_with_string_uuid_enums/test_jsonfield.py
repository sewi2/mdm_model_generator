...
class TestModelGeneratorJsonStringUUIDField(ModelGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "models.JSONField(verbose_name='Oidp uids', "
            "help_text=_('Один объект учета "
            'может сотвествовать нескольким объектам '
            'инвестиционной деятельности, тк '
            "после регистрации почтового адреса объекты "
            "могут быть объединены'), blank=True, null=True, default=dict, "
            "editable=False, db_column='test_field' "
            "if getattr(settings, 'MDM_MODELS_CAMEL_CASE', False) else None)")


class TestSerializerGeneratorJsonStringUUIDField(SerializerGeneratorMixin):
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "serializers.JSONField(label=_('uid-ы ОИДП'), "
            "help_text='Oidp uids', "
            'allow_null=True, default=dict)')
