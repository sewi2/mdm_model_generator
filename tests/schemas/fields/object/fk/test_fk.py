...
class TestModelGeneratorFKField(ModelGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "models.ForeignKey(verbose_name=_('Пользователь'), "
            "help_text=_('Пользователь'), blank=True, "
            "null=True, default=None, editable=False, "
            "db_column='test_fieldGUID' if getattr(settings, "
            "'MDM_MODELS_CAMEL_CASE', False) else None, "
            "on_delete=models.CASCADE, "
            "related_name='testmodel_test_field')")


class TestSerializerGeneratorFKField(SerializerGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            "LazyField(label=_('Пользователь'), help_text=_('Пользователь'), "
            "allow_null=True, path='BaseSecOpsCorpUserSerializer')")
