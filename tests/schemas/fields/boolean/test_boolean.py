...
class TestModelGeneratorBooleanField(ModelGeneratorMixin):
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            'models.BooleanField(verbose_name=_(\'Булево\'), '
            'help_text=_("Пример значения '
            '\'boolean\'"), blank=True, null=True, default=None, '
            'editable=False, '
            "db_column='test_field' if getattr("
            "settings, 'MDM_MODELS_CAMEL_CASE', False) "
            'else None)')


class TestSerializerGeneratorBooleanField(SerializerGeneratorMixin):
    schema: dict = SCHEMA
    ...
    def test_result_definition(self):
        assert self._result_definition == (
            'serializers.BooleanField(label=_(\'Булево\'), '
            'help_text=_("Пример значения '
